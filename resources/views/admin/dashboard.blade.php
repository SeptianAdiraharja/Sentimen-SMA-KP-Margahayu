...
<div class="bg-white p-4 rounded-2xl shadow-sm border border-slate-200 flex flex-col md:flex-row items-center justify-between gap-4">
    <form method="GET" action="{{ route('admin.dashboard') }}" class="flex items-center gap-3 w-full md:w-auto">
        ...
        <select name="period_id" onchange="this.form.submit()" class="text-sm font-semibold text-slate-800 bg-slate-50 border border-slate-200 rounded-xl px-3.5 py-2 focus:ring-2 focus:ring-blue-800 focus:outline-none">
    ...
    <div class="flex items-center gap-2 w-full md:w-auto justify-end">
        <form action="{{ route('admin.analysis.run') }}" method="POST">
            ...
            <button type="submit" class="px-4 py-2 bg-blue-900 hover:bg-blue-800 text-white font-semibold text-xs rounded-xl shadow-sm flex items-center gap-2 transition">
                <i class="fa-solid fa-arrows-rotate"></i>
                <span>Jalankan Ulang Analisis Sentimen</span>
            </button>
        </form>

        <a href="{{ route('admin.reports.index', ['period_id' => $activePeriod?->id]) }}" class="px-4 py-2 bg-amber-400 hover:bg-amber-300 text-blue-950 font-bold text-xs rounded-xl shadow-sm flex items-center gap-2 transition">
            ...
            <tbody class="divide-y divide-slate-100">
                ...
                <tr class="hover:bg-slate-50/80 transition">
                    ...
                    <td class="p-3 text-right">
                        <a href="{{ route('admin.analysis.index', ['aspect_id' => $item['id'], 'period_id' => $activePeriod?->id]) }}" class="px-2.5 py-1 text-xs font-semibold rounded bg-slate-100 hover:bg-blue-100 text-slate-700 hover:text-blue-900 transition">
                            Lihat Jawaban
                        </a>
                    </td>
                </tr>
    ...
    <div class="bg-white p-6 rounded-2xl border border-slate-200 shadow-sm flex flex-col justify-between">
        <div>
            <div class="flex items-center justify-between mb-4">
                ...
                <a href="{{ route('admin.analysis.metrics') }}" class="text-xs font-bold text-blue-800 hover:text-blue-950 hover:underline">Kelola Data Latih &rarr;</a>
            </div>
        ...
        <div class="mt-4 pt-3 border-t border-slate-100 flex items-center justify-between text-xs text-slate-500">
            ...
            <span class="text-blue-800 font-semibold"><i class="fa-solid fa-check"></i> Model Siap Digunakan</span>
        </div>
    </div>
</div>
@endsection
